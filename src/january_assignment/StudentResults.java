...
public class StudentResults {
	...
	public static void main(String[] args) {
		try {
			...
			StudentResults sr = new StudentResults(); // インスタンス
			Object[][] student = new Object[6][10]; // 生徒の名前と各種教科、平均と合計を入れる配列生成
			
			// randomで名前を生成して1列目に格納、各教科得点入力受付
			sr.getRandomNameGeneration(student, ccq1, sc);

			// 各教科の平均点を取得
			sr.getEachAveragePointIndicates(student);

			// 全生徒表示
			sr.getStudentResult(student);
	 ...
	 * */
	public void getStudentResult(Object[][] testData) throws Exception {
		System.out.println("名前                国語    数学    英語    理科    社会  3教科合計 平均  5教科合計 平均");
	...
	public void getRandomNameGeneration(Object[][] testData, ColumnChartQuestion1 ccq1, Scanner sc) throws Exception {
		for (int intStudentPosition = 0; intStudentPosition < 6; intStudentPosition++) {
			
			// ランダムで名前を生成
			testData[intStudentPosition][0] = setCreateRandomName();
			System.out.printf("%sさんの各テスト点数を数値で入力してください\n", (String) testData[intStudentPosition][0]);
			...
			for (int j = 1; j < 6; j++) {
				...
				// 要素数入力受付・規制処理
				testData[intStudentPosition][j] = Integer.parseInt(ccq1.getRequiredInformation(Constant.REGULATION6, Constant.MESSAGE6, Constant.QUESTION6,
						subject, Constant.REGEX_MATCH, sc));
			}

			// 3・5教科合計平均算出結果取得
			getSumAndAverageCalculation(testData, intStudentPosition);
		}
	}
	...
	public void getSumAndAverageCalculation(Object[][] testData, int position) throws Exception {
		...
		// ５教科それぞれの点数を合計していく
		for (int subjects = 1; subjects <= 5; subjects++) {
			sum5Score += (int) testData[position][subjects];

			// ３教科までの合計
			if (subjects <= 3) {
				sum3Score += (int) testData[position][subjects];
			}
		}
